...
class TestRuggedIndex < AmpTestCase
  include Amp::Core::Repositories::Rugged
  INPUT_FILE = "index"
  ...
  def setup
    ...
    @index = Amp::Core::Repositories::Rugged::Index.parse(INPUT_FILE, @opener)
  end
  ...
  def test_failed_fourcc
    ...
    assert_raises Index::IndexParseError do
      Amp::Core::Repositories::Rugged::Index.parse("fakeindex", opener)
    end
  end
